...
#Exract feature - 5-day prediction window based on 'BTC_Weighted_Price'
n,d = data.shape

data['delta_5day']=pd.Series((np.ones(n)), index=data.index)
data['delta_flg']=pd.Series((np.ones(n)), index=data.index)

for i in range(n-5):
    delta = data.iat[i+5, 5] - data.iat[i, 5]
    data.iat[i, d] = delta
    data.iat[i, d+1] = (delta > 0).astype('int')
#trim last 5 rows
data = data.head(n-5)
#print data.head(3)
print data.tail(2)
